...
- `hledger.sh` - finances: auto-format staged journals (hledger-fmt), ban
...
1. Write `<name>.sh` (sourced with `$ROOT` = repo root, `$HOOKS_DIR` set;
   `exit 1` aborts the commit).
2. Add a line to the case block in `pre-commit`:
   `myrepo)  source "$HOOKS_DIR/myrepo.sh" ;;`
